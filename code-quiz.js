...
function startTimer(time){
    ...
    function timer(){
        ...
        if(time < 0){ //if timer is less than 0
            ...
            for(i=0; i < allOptions; i++){
                option_list.children[i].classList.add("disabled"); //once user selects an option then disabled all options
            }
            next_btn.classList.add("show"); //show the next button if user selected any option
        }
...
function startTimerLine(time){
    ...
    function timer(){
        ...
        if(time > 549){ //if time value is greater than 549// clearInterval(counterLine); //clear counterLine
        }
    }  
}
...
function questionCounter(index){
    ...
    bottom_question_counter.innerHTML = totalQuestionCounTag;  //adding new span tag inside 
}
